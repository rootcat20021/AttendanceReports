...
fileToSend = 'attendance_count.pkl'
...
display.start()
download_directory = '/home/rootcat/Downloads/AttendanceCount'
options = webdriver.ChromeOptions() 
preferences = {"download.default_directory": 'Downloads/AttendanceCount',"download.directory_upgrade":"true","default_content_setting_values.automatic_downloads":2}
...
try:
    ...
    element = WebDriverWait(driver, 10).until(lambda x: x.find_element_by_id("ctl00_ContentPlaceHolder1_ddlArea_AttendanceCount"))
    area_drop_down = driver.find_element_by_id('ctl00_ContentPlaceHolder1_ddlArea_AttendanceCount')
    area_drop_down.click()
    area_drop_down.send_keys(Keys.DOWN + Keys.ENTER)
    time.sleep(5)
    element = WebDriverWait(driver, 10).until(lambda x: x.find_element_by_id("ctl00_ContentPlaceHolder1_ddlCentre_AttendanceCount"))
    center_drop_down = driver.find_element_by_id('ctl00_ContentPlaceHolder1_ddlCentre_AttendanceCount')
    center_drop_down.click()
    center_drop_down.send_keys(Keys.DOWN + Keys.ENTER)
    time.sleep(5)
    element = WebDriverWait(driver, 10).until(lambda x: x.find_element_by_id("ctl00_ContentPlaceHolder1_ddlDept_AttendanceCount"))
    dept_drop_down = driver.find_element_by_id('ctl00_ContentPlaceHolder1_ddlDept_AttendanceCount')
    dept_drop_down.click()
    dept_drop_down.send_keys(Keys.DOWN + Keys.ENTER)

    fetch_command = driver.find_element_by_id('ctl00_ContentPlaceHolder1_btnFetch_AttendanceCount')
    fetch_command.click()


    time.sleep(180)
    img = display.waitgrab()
    img.save('/tmp/successful_fetch_count.png')
    driver.quit()
    ...
    try:
        ...
        for filename in filenames:
            ...
            if datetime.datetime.strptime(time.ctime(mtime), "%a %b %d %H:%M:%S %Y") > now.replace(hour=0, minute=0, second=0):
                ...
                df.to_excel('debug_todays_attendance.xlsx')
                df.to_pickle(fileToSend)
                msg["Subject"] = "SENDING_INCREMENTAL_UPDATE:COUNT: " + now.strftime("%Y-%m-%d")
    ...
    print(str(e))
    img = display.waitgrab()
    img.save('/tmp/failed_fetch_count.png')
    driver.quit()
    #time.sleep(DownloadRate) # Let the user actually see something!
